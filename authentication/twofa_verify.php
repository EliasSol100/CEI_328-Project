...
function sendTwoFACode(mysqli $conn, int $userId, string &$errorOut, string &$mailerDetailsOut): ?string
{
    ...
    $stmt = $conn->prepare("SELECT email, full_name FROM users WHERE userID = ?");
    if (!$stmt) {
    ...
    // Save code + expiry
    $upd = $conn->prepare("UPDATE users SET twofa_code = ?, twofa_expires = ? WHERE userID = ?");
    if (!$upd) {
...
if (isset($_POST["verify"])) {
    ...
    if ($code === '') {
        ...
        if ($user = $result->fetch_assoc()) {
            ...
            $clear = $conn->prepare("UPDATE users SET twofa_code = NULL, last_login = NOW() WHERE userID = ?");
