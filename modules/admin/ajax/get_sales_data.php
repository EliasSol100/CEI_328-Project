...
if ($productsRes) {
    while ($row = mysqli_fetch_assoc($productsRes)) {
        ...
        if (array_key_exists($pid, $manualSalesMap)) {
            $baseline = $manualSalesMap[$pid]['auto_sales_baseline'] ?? null;
            if ($baseline === null) {
                $baseline = $autoSales;
            }
            $result[$pid] = (int)$manualSalesMap[$pid]['manual_total_sales'] + max(0, $autoSales - (int)$baseline);
        } else {
            $result[$pid] = $autoSales;
        }
    }
}
